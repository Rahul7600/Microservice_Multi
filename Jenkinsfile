    ...
    stages {
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                        sh "docker build -t asia-south1-docker.pkg.dev/kubernetes-470606/repos/emailservice:latest ."
                    }
                }
            }
        }
        ...
        stage('Docker Image Push to GCP Artifacts') {
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                        sh "docker push asia-south1-docker.pkg.dev/kubernetes-470606/repos/emailservice:latest"
                    }
                }
            }
        }
